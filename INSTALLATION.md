...
⚠️ **Важно**: Проект должен быть установлен в /opt/remnawave-STEALTHNET-Panel/ (клонирован из GitHub репозитория).

Создаем директорию проекта и клонируем проект из GitHub
```bash
cd /opt
...
```env
JWT секретный ключ
# Ниже описаны методы генерации ключей
...
# URL для Mini-App совпадает с адресом StealthNET Admin Panel
...
Замените на ваш адрес StealthNET Admin Panel или на IP вашего сервера
```bash
...
Откройте в браузере:
- `http://IP вашего сервера` или `https://ваш адрес StealthNET Admin Panel`
...
docker compose logs
```
Проверяем статус контейнеров проекта
```bash
docker compose ps
```
